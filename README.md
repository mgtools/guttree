...
Having a list of peptides, we created a script that will search these pepetides against a list of protein sequences coming from the genomes participating in constructing the phylogenetic tree. To do this we wrote a script that does exact matching between the peptides and proteins. The script can be found [here](metaproteome_application/peptide2sequenceStringMatching.py), which takes 3 command line arguments to run:

1) directory to the peptide file
2) directory to the protein sequences of the genomes (in one multifasta file)
3) directory to store the output file generated by the script

example to run this script

```
python3 peptides2sequenceStringMatching.py peptideSeqs_unique.fasta ../data/final_genomesAllProteins.faa ../metaproteome_application/
```

#### Peptide to genome map counts

This step involves quantifying the number of peptides being mapped to the genomes. To do this we 
...
Once we haev a reads mapping file available, we made a script that will quantify the relative abundances of each of the participating genomes in our study. We made a script that does that which could be accessed [here](metagenome_application/samRead2multiFastaGenomeAbundance.py). This script requires 3 command line arguments to run:
