...
For the Metaproteomic application of your gut tree, we expect the user to provide a list of peptides sequences in fasta format to search and quantify over our tree. Peptides could be obtained by any search method which is beyond the scope of our study. We made a script that will read in peptide sequences in fasta format and will produce the necessarry iTOL importable annotation files to reflect these informations on out tree. This script could be found [here](metaproteome_application/quantifyMP.sh), that takes four command line arguments to run:
