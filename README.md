...
For the user's convenience we also included a script that will combine the three steps mentioned in this section in one command. This script under the name [quantifyMP.sh](metaproteome_application/quantifyMP.sh), executes all the steps mentioned in this section by providing it with four command line arguments:

1) the peptide sequence file (in fasta format)
3) output directory to store files generated by the scripts
3) number of cores allocated to run the script

example to run script:

```
sh quantify_mp.sh -p ../dir/to/pep/seqs.fasta -o /out/dir/ -t 40
```
